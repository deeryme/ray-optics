...
for h in [1.0]:
    ...
    mir_h_str = 'f'.join(f"{h:.4f}".split('.'))
    scene['name'] = f"reflector_w_{mir_w_str}mm_h_{mir_h_str}mm"
    # h_file_names.append(scene['name'])
    ...
    for idx, pos in enumerate(x): 
        ...
        scene["objs"][3]["p1"]["x"] = -my_scene.mirror_w/2 + pos
        scene["objs"][3]["p1"]["y"] = -h
        scene["objs"][3]["p2"]["x"] =  my_scene.mirror_w/2 + pos
        scene["objs"][3]["p2"]["y"] = -h
        scene["objs"][5]["text"] = f"top = {h} mm,\nMirror is\n{pos:.2f} mm\nfrom ctr"       
...
print("\nExamples completed!")
# with open(f"{main_dir_name}/heights.json", 'w') as f:
#     json.dump(h_file_names, f)
