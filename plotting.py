...
def plot_power_v_displacement(x, y, P, main_dir_name):
    ...
    ax_3d.plot_surface(xx, yy, np.abs(P), cmap=cm.coolwarm)
    ...
    plt.contourf(xx, yy, P, cmap=cm.coolwarm)
    plt.colorbar()
    plt.grid(True)
...
def get_arc_mirr_plots(mirr_w, r, d, is_concave_up):
    ...
    main_dir_name = f"../results/arc/concave_{concavity}_r_{mir_r_str}mm"
    load_json_power_data(main_dir_name, P)    
